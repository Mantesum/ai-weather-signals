...
`scripts/setup.ps1` supports the default Docker Desktop installation. This deployment instead uses the PostgreSQL server configured by `DATABASE_URL` in `.env`; TLS can be required with `sslmode=require`. The native CUDA llama.cpp server avoids WSL/Docker GPU uncertainty on the audited V100 machine.
