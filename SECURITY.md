...
Untrusted provider text is data, never a prompt instruction: the system prompt limits output, the configured LLM provider receives a JSON schema, and Pydantic validates again. URLs are not fetched during evidence serving. Production should add outbound allow-lists, request-body limits, API authentication/rate limiting, dependency scanning and source-specific deletion consumers.
