...
- Google News RSS: multiple regional queries merged into bounded logical sources. The deployment uses 97 queries in 19 groups with equal one-hour regional intervals. Coverage includes Africa, North America, Latin America and the Caribbean, Europe, the Middle East and North Africa, South Asia, East Asia, Central Asia and the Caucasus, Southeast Asia, and Oceania/Pacific. Local-language terms are used alongside English, feed HTML is stripped, and every returned batch is capped. This is a useful discovery layer but not a documented Google API or an official confirmation source.
...
- NWS: the official nationwide active-alert Atom feed is bounded to 40 items per cycle and treated as a high-trust source. BBC remains a lower-trust discovery source. The retired Reuters environment RSS endpoint is kept disabled for configuration history.
